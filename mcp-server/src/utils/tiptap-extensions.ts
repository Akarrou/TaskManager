...
export const AccordionTitle = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return ['summary', { class: 'accordion-title', style: `color:${HTMLAttributes.titleColor as string}` }, 0];
  },
});
...
export const EnhancedImage = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    if (attrs.src) imgAttrs.src = attrs.src;
...
export const DatabaseTable = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const config = attrs.config as Record<string, string> | undefined;
    const name = config?.name || 'Database';
    const dbId = attrs.databaseId as string;
...
export const SpreadsheetNode = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const ssId = attrs.spreadsheetId as string;
...
export const MindmapNode = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    const attrs = HTMLAttributes as Record<string, unknown>;
    const mindmapId = attrs.mindmapId as string;
