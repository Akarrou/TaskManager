  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (!isRetryable || attempt === maxAttempts) {
        logger.error(
          { operationName, attempt, maxAttempts, error: lastError.message, retryable: isRetryable },
      ...
      logger.warn(
        { operationName, attempt, maxAttempts, delay, error: lastError.message },
...
export async function testConnection(): Promise<boolean> {
  try {
    ...
    if (error) {
      logSupabase('testConnection', 'projects', duration, error as unknown as Error);
      return false;
    }
    ...
    logSupabase('testConnection', 'projects', undefined, error as Error);
    return false;
  }
}
...
export async function executeWithRetry<T>(
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    logSupabase(operationName, tableName, duration, error as Error);
