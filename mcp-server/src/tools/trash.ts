...
export function registerTrashTools(server: McpServer): void {
  ...
  server.tool(
    'list_trash',
    `List all items currently in the user's trash. Items are kept for 30 days before automatic permanent deletion. Each item shows its type, name, original location, deletion date, and expiration date. Use restore_from_trash to recover items or permanent_delete_from_trash to delete them immediately.`,
    {
  ...
  server.tool(
    'restore_from_trash',
    `Restore a previously deleted item from the trash. This clears the deleted_at timestamp on the original item and removes it from the trash. The item will reappear in its original location. Use list_trash first to find the trash_id.`,
    {
  ...
  server.tool(
    ...
    `DESTRUCTIVE: Permanently delete an item from the trash. This performs a hard delete on the original table and removes the trash record. This action cannot be undone. Use list_trash first to find the trash_id.`,
    {
      trash_id: z.string().uuid().describe('The trash item ID (from list_trash output).'),
  ...
  server.tool(
    'empty_trash',
    `DESTRUCTIVE: Permanently delete ALL items from the trash. This hard deletes every item from their original tables and clears the entire trash. This action cannot be undone.`,
    {
