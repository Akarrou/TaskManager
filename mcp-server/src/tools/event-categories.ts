...
export function registerEventCategoryTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        const custom = (customCategories || []).map((c: Record<string, unknown>) => ({
          key: c.key,
          label: c.label,
  ...
  server.tool(
    ...
    async ({ label, color_key }) => {
      try {
        ...
        const sortOrder = ((maxOrder?.sort_order as number) || 0) + 1;
  ...
  server.tool(
    ...
    `Update an existing custom event category. Only custom categories can be updated — default categories (meeting, deadline, milestone, reminder, personal, other) cannot be modified. You can update the label, color_key, or both. At least one update field must be provided.`,
    {
    ...
    async ({ key, label, color_key }) => {
      try {
        ...
        if (color_key !== undefined) updateData.color_key = color_key;
