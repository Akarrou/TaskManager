...
export function registerEventTools(server: McpServer): void {
  ...
  server.tool(
    'create_event',
    `Create a new event in an event-type database. A linked document is automatically created (Notion-style).
...
Returns: { event, document }. Related tools: list_databases, create_database, list_calendar_docs.`,
    ...
    async ({ database_id, title, description, start_date, end_date, all_day, category, location, recurrence, linked_items, attendees, guest_permissions, reminders, meet_link, color, project_id, user_id }) => {
      try {
        ...
        if (startDateColId) cells[startDateColId] = start_date;

        const endDateColId = findColumnId('End Date');
        if (endDateColId) cells[endDateColId] = end_date;

        const allDayColId = findColumnId('All Day');
        if (allDayColId) cells[allDayColId] = all_day;

        const categoryColId = findColumnId('Category');
        if (categoryColId) cells[categoryColId] = category;

        const locationColId = findColumnId('Location');
        if (locationColId && location) cells[locationColId] = location;

        const recurrenceColId = findColumnId('Recurrence');
        if (recurrenceColId && recurrence) cells[recurrenceColId] = recurrence;
  ...
  server.tool(
    'get_event_document',
    `Get the document linked to an event. Each event in an event database has an associated document (Notion-style) that can contain rich content, notes, and details. The document is automatically created when the event is created. Use this to retrieve the document for viewing or editing its content. Returns the full document object including id, title, content, and metadata.`,
    {
  ...
  server.tool(
    ...
    async ({ database_id, row_id, title, description, start_date, end_date, all_day, category, location, recurrence, linked_items, attendees, guest_permissions, reminders, meet_link, color }) => {
      try {
        ...
        if (start_date !== undefined) updates.push({ name: 'Start Date', value: start_date });
        if (end_date !== undefined) updates.push({ name: 'End Date', value: end_date });
        if (all_day !== undefined) updates.push({ name: 'All Day', value: all_day });
        if (category !== undefined) updates.push({ name: 'Category', value: category });
  ...
  server.tool(
    'delete_event',
    `Move an event to the trash (soft delete). The event is marked as deleted but can be restored within 30 days using restore_from_trash. A snapshot is taken before deletion for additional recovery. Note: if Google Calendar sync is active, the corresponding Google event will also be deleted when the event is removed via the Angular app.`,
    {
 ...
 */
function normalizeRowToEvent(row: Record<string, unknown>, dbMeta: Record<string, unknown>): Record<string, unknown> {
  const config = dbMeta.config as { columns?: Array<{ id: string; name: string }> };
  const columns = config.columns || [];
 ...
 */
function parseReminders(value: string | null | undefined): unknown[] | null {
