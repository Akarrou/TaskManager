...
export function registerTiptapDocsTools(server) {
    server.tool('list_tiptap_extensions', `List all custom TipTap extensions available in the Kodo editor. Call this to discover which extensions exist before retrieving their documentation. Related tools: get_tiptap_extension_doc (full documentation for one extension).`, {}, async () => {
        const list = VALID_EXTENSIONS.map((name) => `- **${name}**: ${EXTENSION_DESCRIPTIONS[name]}`).join('\n');
        return {
            content: [
                {
                    type: 'text',
...
//# sourceMappingURL=tiptap-docs.js.map