...
export function registerDatabaseTools(server) {
    ...
    // =========================================================================
    server.tool('get_database_schema', `Get the complete schema of a database including all column definitions. Returns: database_id, name, type, columns array, views, defaultView, and pinnedColumns.
...
Example cells: { "Title": "My Item", "Status": "pending", "Priority": "high", "Count": 42 }. Related tools: get_database_schema (column names and choice IDs), update_database_row.`, {
    ...
    // =========================================================================
    server.tool('update_database_row', `Update specific cells in an existing database row. Only provide the columns you want to change - existing values are preserved for unspecified columns. Uses column names (not IDs) in the cells object. For task databases, use update_task instead. Returns the updated row.
...
Example: { "Status": "completed", "Priority": "high", "Progress": 100 }. Related tools: get_database_rows (find row IDs), add_database_row, delete_database_rows.`, {
    ...
    // =========================================================================
    server.tool('create_database', `Create a new Notion-like database with custom columns. Databases are structured tables that can be embedded in documents or exist standalone.
...
Returns the created database with its generated database_id. Related tools: add_column (add more columns later), add_database_row (add data), list_tasks/create_task (for task databases).`, {
        ...
        try {
            ...
            if (type === 'task' && (!columns || columns.length === 0)) {
                ...
                columnsConfig.push({ id: titleId, name: 'Title', type: 'text', visible: true, required: true, readonly: true, order: 0, width: 200, color: 'blue' }, { id: descriptionId, name: 'Description', type: 'text', visible: true, readonly: true, order: 1, width: 300, color: 'green' }, { id: statusColumnId, name: 'Status', type: 'select', visible: true, readonly: true, order: 2, width: 180, color: 'yellow', options: {
                        choices: [
                    ...
                    } }, { id: assignedToId, name: 'Assigned To', type: 'text', visible: true, readonly: true, order: 5, width: 200, color: 'pink' }, { id: dueDateId, name: 'Due Date', type: 'date', visible: true, readonly: true, order: 6, width: 150, color: 'orange', options: { dateFormat: 'DD/MM/YYYY' } }, { id: tagsId, name: 'Tags', type: 'multi-select', visible: true, readonly: true, order: 7, width: 220, color: 'gray', options: {
                        choices: [
    ...
    // =========================================================================
    server.tool('add_column', `Add a new column to an existing database. The column is added at the end with auto-generated UUID as ID and auto-assigned order. Existing rows will have null values for the new column until updated.
...
Column names must be unique within the database. Returns the created column with its generated UUID. Related tools: update_column, delete_column, get_database_schema.`, {
    ...
    // =========================================================================
    server.tool('update_column', `Update a column's properties (name, visibility, select options). Column type cannot be changed. Only provide the properties you want to change. For select/multi-select, provide options as [{label, color?}] — they are automatically converted to {choices: [{id, label, color}]} format. Hidden columns (visible=false) don't appear in default table views but data is preserved. Get column_id from get_database_schema. Related tools: add_column, delete_column.`, {
        database_id: z.string().describe('The database ID. Format: db-uuid.'),
