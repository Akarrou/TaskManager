 ...
 */
async function userHasTabAccess(supabase, tabId, userId) {
...
export function registerTabTools(server) {
    ...
    // =========================================================================
    server.tool('list_tabs', `List all tabs for a project. Tabs are the primary organizational structure in the project sidebar - they group related documents and content. Each tab has a name, optional icon/color, and position. Tabs can belong to tab groups for further organization. Returns tabs sorted by position. Use this to understand project structure before creating documents or sections. Related tools: create_tab, list_sections (content within tabs).`, {
    ...
    server.tool('create_tab', `Create a new tab in a project's sidebar. Tabs organize content hierarchically - after creating a tab, add sections to it, then documents within sections. Position is auto-assigned to the end. Can optionally be placed in a tab group for categorization. Returns the created tab with its ID. Typical workflow: create_tab -> create_section -> create_document. Related tools: list_tabs, create_section, update_tab.`, {
        ...
        try {
            ...
            if (color)
                tabData.color = color;
            if (tab_group_id)
                tabData.tab_group_id = tab_group_id;
            const { data, error } = await supabase
                .from('document_tabs')
                .insert(tabData)
                .select()
    ...
    // =========================================================================
    server.tool('update_tab', `Update a tab's display properties. Only provide fields you want to change. Use tab_group_id=null to remove from a group. Position changes require reorder_tabs. Returns the updated tab. Related tools: reorder_tabs (change order), delete_tab, list_tabs.`, {
        ...
        try {
            ...
            if (tab_group_id !== undefined)
                updates.tab_group_id = tab_group_id;
    ...
    // =========================================================================
    server.tool('delete_tab', `Delete a tab from a project. WARNING: This also removes all sections within the tab. Documents may become orphaned (not linked to any tab). Consider the impact on project organization before deleting. Default tabs cannot be deleted - change the default first using set_default_tab. Returns confirmation of deletion.`, {
    ...
    // =========================================================================
    server.tool('reorder_tabs', `Reorder tabs in the sidebar by providing the complete ordered list of tab IDs. Position 0 is at the top. All tabs to be reordered should be included in the array - missing tabs retain their position. Use list_tabs first to get current tab IDs and order. Returns success count.`, {
    ...
    // =========================================================================
    server.tool('create_tab_group', `Create a new tab group in a project to organize tabs. Tab groups appear as collapsible sections in the sidebar. After creating a group, use update_tab with tab_group_id to add tabs to it. Position is auto-assigned to the end. Groups start expanded (is_collapsed=false). Related tools: list_tab_groups, update_tab_group, update_tab.`, {
    ...
    // =========================================================================
    server.tool('update_tab_group', `Update a tab group's properties. Only provide fields you want to change. Set is_collapsed=true to collapse the group (hide its tabs in the sidebar UI). Returns the updated group. Related tools: delete_tab_group, list_tab_groups.`, {
        ...
        try {
            ...
            if (is_collapsed !== undefined)
                updates.is_collapsed = is_collapsed;
            if (Object.keys(updates).length === 1) {
                return {
                    content: [{ type: 'text', text: 'No updates provided.' }],
                    isError: true,
    ...
    // =========================================================================
    server.tool('delete_tab_group', `Delete a tab group from a project. SAFE: Tabs that belong to this group are NOT deleted - they become ungrouped (tab_group_id set to null) and remain visible in the sidebar. Only the grouping is removed. Returns confirmation of deletion.`, {
    ...
    // =========================================================================
    server.tool('create_section', `Create a new section within a tab to organize documents. Sections act as headings/dividers that group related documents. Position is auto-assigned to the end. Sections start expanded. After creating a section, you can link documents to it. Typical hierarchy: Project -> Tab -> Section -> Document. Related tools: list_sections, update_section, create_document.`, {
    ...
    // =========================================================================
    server.tool('update_section', `Update a section's properties. Only provide fields you want to change. Set is_collapsed=true to collapse the section (hide documents under it in the UI). Returns the updated section. Related tools: delete_section, list_sections.`, {
    ...
    // =========================================================================
    server.tool('delete_section', `Delete a section from a tab. Documents in the section may become orphaned (not linked to any section) but are not deleted. The section heading and organization is removed. Returns confirmation of deletion. Related tools: list_sections, create_section.`, {
