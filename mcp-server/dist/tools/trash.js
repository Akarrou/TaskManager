 ...
 */
export function registerTrashTools(server) {
    // =========================================================================
    ...
    server.registerTool('list_trash', {
        ...
        try {
            ...
            if (!data || data.length === 0) {
                return {
                    content: [{ type: 'text', text: 'Trash is empty.' }],
                };
            }
            const items = data.map((item) => {
                ...
                const parentInfo = item.parent_info;
    ...
    server.registerTool('restore_from_trash', {
        ...
        try {
            ...
            if (fetchError || !trashItem) {
                return {
                    content: [{ type: 'text', text: `Trash item not found: ${trash_id}` }],
                    isError: true,
                };
            }
    ...
    server.registerTool('empty_trash', {
        ...
        if (!confirm) {
            return {
                content: [{ type: 'text', text: 'Not confirmed. Set confirm=true to proceed.' }],
                isError: true,
            };
        }
        try {
            ...
            if (!items || items.length === 0) {
                return {
                    content: [{ type: 'text', text: 'Trash is already empty.' }],
                };
            }
            let deletedCount = 0;
            const errors = [];
