...
export function registerEventTools(server) {
    ...
    // =========================================================================
    server.tool('list_events', `List events aggregated from all event-type databases in the workspace. Events are stored as rows in "event" type databases with standardized columns: Title, Description, Start Date, End Date, All Day, Category, Location, Recurrence, Linked Items, Project ID, Event Number. This tool scans all event databases and returns a unified view. Results are normalized and sorted by start date. Category values: meeting, deadline, milestone, reminder, personal, other. Related tools: create_event, update_event, get_event.`, {
...
Returns: { event, document }. Related tools: list_databases, create_database.`, {
    ...
    // =========================================================================
    server.tool('update_event', `Update one or more fields of an existing event. Only provide the fields you want to change - unspecified fields remain unchanged. Returns the complete updated event. At least one field must be provided. All field values use the same format as create_event.`, {
        database_id: z.string().describe('The database ID containing the event. Format: db-uuid.'),
    ...
    // =========================================================================
    server.tool('delete_event', `Permanently delete an event from a database. This removes the row from the event database. The deletion is immediate and cannot be undone. Returns confirmation with the deleted event's information. A snapshot is taken before deletion for recovery purposes.`, {
